...
RGBQUAD CxImage::RGBtoHSL(RGBQUAD lRGBColor)
{
	...
	uint8_t cMax,cMin;				/* cxmax and cxmin RGB values */
	uint16_t Rdelta,Gdelta,Bdelta;	/* intermediate value: % of spread from cxmax*/
					...
					case OpBlendAlpha:	//[brunom]
						if(rgb2.rgbReserved != 0)
						{
							...
							if( (rgb1.rgbReserved < 5) || (rgb2.rgbReserved > 250) ){
								...
								rgbDest.rgbGreen	= (char)((rgb2.rgbGreen * a2	+ a1 * rgb1.rgbGreen)/a0);
								rgbDest.rgbRed		= (char)((rgb2.rgbRed	* a2	+ a1 * rgb1.rgbRed	)/a0);
							}
						} else {
							rgbDest = rgb1;
 ...
 * \param amount: strength of the filter. Range: 0.0 (none) to 1.0 (cxmax); default = 0.5
 ...
 * \param method: 0 = average all methods (default); 1 = Otsu; 2 = Kittler & Illingworth; 3 = cxmax entropy; 4 = potential difference;
 ...
 * \param method: 0 = average all methods (default); 1 = Otsu; 2 = Kittler & Illingworth; 3 = cxmax entropy; 4 = potential difference;
 * \param nBoxSize: the image is divided into "nBoxSize x nBoxSize" blocks, from where the threshold is computed; cxmin = 8; default = 64.
 * \param pContrastMask: limit the computation only in regions with contrasted (!=0) pixels; default = 0.
...
//------------------------------------- Begin of Flood Fill
	...
	if (IsIndexed()){ //--- Generic indexed image, no tolerance OR Grayscale image with tolerance
		...
		uint8_t idxMin = (uint8_t)cxmin(255, cxmax(0, (int32_t)(idxRef - nTolerance)));
		uint8_t idxMax = (uint8_t)cxmin(255, cxmax(0, (int32_t)(idxRef + nTolerance)));
		...
		cRefMin.rgbRed   = (uint8_t)cxmin(255, cxmax(0, (int32_t)(cRef.rgbRed - nTolerance)));
		cRefMin.rgbGreen = (uint8_t)cxmin(255, cxmax(0, (int32_t)(cRef.rgbGreen - nTolerance)));
		cRefMin.rgbBlue  = (uint8_t)cxmin(255, cxmax(0, (int32_t)(cRef.rgbBlue - nTolerance)));
		cRefMax.rgbRed   = (uint8_t)cxmin(255, cxmax(0, (int32_t)(cRef.rgbRed + nTolerance)));
		cRefMax.rgbGreen = (uint8_t)cxmin(255, cxmax(0, (int32_t)(cRef.rgbGreen + nTolerance)));
		cRefMax.rgbBlue  = (uint8_t)cxmin(255, cxmax(0, (int32_t)(cRef.rgbBlue + nTolerance)));
